...
def test_archive_success(mock_archiver_dependencies, tmp_path):
...
def test_archive_no_file(mock_archiver_dependencies):
    mock_dm, mock_logger, config = mock_archiver_dependencies
...
def test_invalid_config_raises(mock_archiver_dependencies):
    mock_dm, mock_logger, _ = mock_archiver_dependencies
...
def test_copy_failure_raises_archiver_error(mock_archiver_dependencies, tmp_path):
    mock_dm, mock_logger, config = mock_archiver_dependencies
...
def test_get_timestamped_path_creates_dir(mock_archiver_dependencies):
    mock_dm, mock_logger, config = mock_archiver_dependencies
    archiver = USGSArchiver(data_manager=mock_dm, logger=mock_logger, config=config)
...
def test_copy_file_copies_content(mock_archiver_dependencies, tmp_path):
    mock_dm, mock_logger, config = mock_archiver_dependencies
    archiver = USGSArchiver(data_manager=mock_dm, logger=mock_logger, config=config)
